...
def optional_bootstrap(args: argparse.Namespace) -> str:
    ...
    if args.with_cyber:
        sections.append(
            ...
            "        binutils binwalk gdb openjdk-21-jdk patchelf \\\n"
            ...
            "      sudo -u \"$admin_user\" -H \"$admin_home/miniforge3/bin/mamba\" create -y -n sage sage\n"
            "      printf '%s\\n' \\\n"
            ...
            "      install -d -o \"$admin_user\" -g \"$admin_user\" \\\n"
            "        \"$admin_home/.local\" \"$admin_home/.local/share\" \"$admin_home/.local/bin\"\n"
            ...
            "      # filebytes only publishes an obsolete source distribution to PyPI.  Its\n"
            "      # setup script uses ast.Str, removed in Python 3.14 (Ubuntu 26.04).\n"
            "      # Use its compatible, pure-Python wheel so pipx never builds that sdist.\n"
            "      sudo -u \"$admin_user\" -H pipx install \\\n"
            "        --pip-args='--extra-index-url https://www.piwheels.org/simple --only-binary=filebytes' \\\n"
            "        ropper\n"
            "      gem install one_gadget\n"
            "      printf '%s\\n' '#!/usr/bin/env bash' \\\n"
            "        'exec gdb -q -ex \"source $HOME/.local/share/gef/gef.py\" \"$@\"' \\\n"
            "        > \"$admin_home/.local/bin/gef\"\n"
            "      chown \"$admin_user:$admin_user\" \"$admin_home/.local/bin/gef\"\n"
            "      chmod 0755 \"$admin_home/.local/bin/gef\"\n"
            ...
            "      \"$ghidra_dir/server/ghidraSvr\" start\n"
    ...
    if args.with_t3code:
        sections.append(
            "      # Codex uses its official standalone installer on Linux.\n"
            "      sudo -u \"$admin_user\" -H bash -lc 'curl -fsSL https://chatgpt.com/codex/install.sh | sh'\n"
            "      # T3 Code's official quick-start method is npx; retain the familiar t3 command.\n"
            "      printf '%s\\n' '#!/usr/bin/env bash' 'exec npx --yes t3@latest \"$@\"' \\\n"
            "        > /usr/local/bin/t3\n"
